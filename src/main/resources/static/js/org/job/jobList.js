    ...
    var pageList = new Vue({
        ...
        methods: {
        	flushData:function(){
    			var self=this;
    			axios.get('/org/orgJob/getOrgJobInfoList').then(response => {
    				self.listData=response.data;
    			});
        	},
        	
            show (index) {
               	var c={title:'编辑职务',url:'/html/org/job/jobEdit.html?key='+index};
            	GPageModel.info(c);
            },
            add:function(){
            	var c={title:'新增职务',url:'/html/org/job/jobEdit.html'};
            	GPageModel.info(c);
            ...
            remove:function(index) {
        		this.$Modal.confirm({
					...
					onOk:function(){
						var self=this;
						self.$Spin.show();
		    			axios.get('/org/orgJob/deleteOrgJobInfo?key='+index).then(response => {
