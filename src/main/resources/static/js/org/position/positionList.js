    ...
    var pageList = new Vue({
        ...
        methods: {
        	flushData:function(){
    			var self=this;
    			axios.get('/org/orgPosition/getOrgPositionInfoList').then(response => {
    				self.listData=response.data;
    			});
        	},
            show (index) {
            	var c={title:'编辑职位',url:'/html/org/position/positionEdit.html?key='+index};
            	GPageModel.info(c);
            },
            add:function(){
            	var c={title:'新增职位',url:'/html/org/position/positionEdit.html'};
            	GPageModel.info(c);
            },
            remove:function(index) {
        		this.$Modal.confirm({
					...
					onOk:function(){
						var self=this;
						self.$Spin.show();
		    			axios.get('/org/orgPosition/deleteOrgPositionInfo?key='+index).then(response => {
